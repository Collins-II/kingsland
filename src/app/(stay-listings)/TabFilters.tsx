...
interface TabProps {
  data: StayDataType[];
  setFilteredData: Dispatch<SetStateAction<StayDataType[]>>;
}
...
// Filter function
const filterProperties = (properties: StayDataType[], filters: FilterOptions): StayDataType[] => {
...
// ✅ Prevent Infinite Loop: Only update filters if values change
useEffect(() => {
  setFilters((prev) => {
    ...
    // 🔹 Only update state if values are different to prevent infinite loop
                    ...
                    {moreFilter3.map((item) => (
                      <div key={item.name} className="">
                        <Checkbox
                          ...
                          onChange={() => dispatch(setPropertyType(item.name))}
                        />
                      </div>
                    ))}
                  </div>
          ...
          <div key={index} className="flex flex-col space-y-5">
            {list.map((item) => (
              <Checkbox
                ...
                onChange={() => handleCheckboxChange(item.name)} // Dispatch updates
              />
            ))}
          </div>
        ))}
                          ...
                          <div className="relative flex flex-col space-y-8">
                            <div className="space-y-5">
                              <Slider
                                ...
                                defaultValue={[0, 1000]}
                                allowCross={false}
                                onChange={(values) => dispatch(setPriceRange(values as [number, number]))} // ✅ Fix: Cast to tuple
                              />
                            </div>
